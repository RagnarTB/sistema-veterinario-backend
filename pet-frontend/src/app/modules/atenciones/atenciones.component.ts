...
export class AtencionesComponent implements OnInit {
  ...
  canRegisterAtencion = this.authService.hasPermission('REGISTRAR_ATENCION');
  ...
  get esVetAsignado(): boolean {
    if (!this.citaSeleccionada) return false;
    // Si es ADMIN, quizá queramos dejarle bypass, pero por reglas médicas estrictas,
    // solo el vet asignado debería guardar atenciones. Si quieres bypass admin:
    // if (this.authService.isAdmin()) return true;
    return this.citaSeleccionada.veterinarioEmail === this.authService.currentEmail();
  }
              ...
              <tbody>
                ${receta.detalles && receta.detalles.length > 0 ? 
                  receta.detalles.map((d: any) => `
                    <tr>
                      <td><strong>${d.medicamento}</strong></td>
                      <td>${d.dosis}</td>
                      <td>${d.frecuencia}</td>
                      <td>${d.duracionDias} días</td>
                    </tr>
                  `).join('') : `
                  <tr>
                    <td colspan="4" style="text-align: center; color: #666; font-style: italic;">
                      Sin medicamentos especificados en esta prescripción.
                    </td>
                  </tr>
                `}
              </tbody>
