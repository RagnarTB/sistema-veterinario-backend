    ...
    <div class="auth-container">
      <div class="auth-card premium-card fade-in-up">
        ...
        <div class="auth-header text-center">
          ...
          <h2 class="auth-title">{{ isReset() ? 'Restablecer Contraseña' : 'Activar tu Cuenta' }}</h2>
          <p class="auth-subtitle">{{ isReset() ? 'Ingresa tu nueva contraseña para acceder al sistema.' : 'Crea una contraseña segura para tu portal de cliente.' }}</p>
        </div>
        ...
        @if (loading()) {
            ...
            <form [formGroup]="form" (ngSubmit)="onSubmit()" class="auth-form">
                ...
                <button mat-flat-button color="primary" class="w-full btn-large" type="submit" [disabled]="form.invalid || isSubmitting()">
                    {{ isSubmitting() ? 'Guardando...' : (isReset() ? 'Restablecer Contraseña' : 'Activar mi Cuenta') }}
                </button>
            </form>
        }
  ...
  onSubmit() {
    ...
    if (this.isReset()) {
      this.authService.resetPassword(this.token, password).subscribe({
        next: (res) => {
          this.successMsg.set(res.mensaje || '¡Contraseña actualizada exitosamente!');
