...
export class CajaComponent implements OnInit {
  ...
  cargarDiario() {
    this.ventaService.listar(0, 500).subscribe({
      next: (res) => {
        // Filtrar transacciones pertenecientes EXCLUSIVAMENTE a la caja activa actual
        // Usando cajaId en lugar de fecha para evitar doble cómputo al reabrir caja el mismo día
        const activaCajaId = this.estadoCaja?.cajaId;
        if (activaCajaId) {
          this.ventasHoy = (res.content || []).filter(v => v.cajaId === activaCajaId);
        } else {
          // Fallback: filtrar por fecha del día actual si no hay cajaId disponible
          const hoy = new Date().toDateString();
          this.ventasHoy = (res.content || []).filter(v => new Date(v.fechaHora).toDateString() === hoy);
        }

        // Sumar monto real pagado de las ventas de esta caja
  ...
  mezclarMovimientos() {
    // Tomamos ventas hoy y les damos un formato homogeneo para el Kardex
    const ventasFormateadas = this.ventasHoy.map(v => {
