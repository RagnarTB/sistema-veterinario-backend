...
export class CitaAgendarDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.authService.hasRole('ROLE_CLIENTE')) {
      this.clienteService.misDatos().subscribe({
        next: (res) => {
          if (res) {
        ...
        error: (err) => {
          this.snack.open('Su sesión ha expirado o hubo un error. Vuelva a iniciar sesión.', 'Cerrar', { duration: 3000 });
        }
      });
    }
    ...
    this.empleadoService.listar(0, 100, '', true, this.data.sedeId).subscribe(res => {
      let vets = res.content.filter(e => e.nombresRoles?.includes('ROLE_VETERINARIO'));
      if (this.authService.hasRole('ROLE_CLIENTE')) {
        const currentUserEmail = this.authService.currentEmail();
        vets = vets.filter(e => e.email !== currentUserEmail);
      }
