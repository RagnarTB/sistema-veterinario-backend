...
export class CitaDetalleDialogComponent {
  ...
  retirarDeAgenda(): void {
    const ref = this.matDialog.open(ModalConfirmacionComponent, {
      width: '400px',
      data: {
        title: this.isCliente ? 'Cancelar Cita' : 'Retirar Cita de Agenda',
        message: this.isCliente 
          ? '¿Está seguro de cancelar esta cita?' 
          : '¿Está seguro de cancelar esta cita agendada? Se retirará de la agenda.',
        confirmText: this.isCliente ? 'Sí, Cancelar' : 'Sí, Retirar',
        isDestructive: true
      }
    });
