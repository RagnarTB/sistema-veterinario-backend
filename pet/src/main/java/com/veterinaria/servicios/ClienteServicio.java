...
public class ClienteServicio {
    ...
    public ClienteResponseDTO guardar(ClienteRequestDTO dto) {
        ...
        if (usuario == null) {
            ...
            if (!usuario.getRoles().contains(rolCliente)) {
                usuario.getRoles().add(rolCliente);
                usuario = usuarioRepositorio.save(usuario);
            }
        }
        ...
        // Si el usuario ya tiene contraseña (ej. empleado activo), nace verificado/activo
        boolean yaVerificado = usuario.getPassword() != null;
        cliente.setActivo(yaVerificado);
        ...
        respuesta.setVerificado(yaVerificado);
    ...
    public ClienteResponseDTO obtenerMiPerfil() {
        if (!usuarioAutenticadoService.esCliente()) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "No tienes rol de cliente");
        }
        Cliente cliente = usuarioAutenticadoService.obtenerClienteActual();
        com.veterinaria.modelos.Usuario u = cliente.getUsuario();
        boolean verificado = u != null && u.getPassword() != null;
        return new ClienteResponseDTO(
            cliente.getId(),
            u != null ? u.getNombre() : "",
            u != null ? u.getApellido() : "",
    ...
    public void cambiarEstado(Long id, Boolean estado) {
        ...
        if (estado) {
            ...
            if (clientedb.getUsuario() != null) {
                clientedb.getUsuario().setActivo(true);
                usuarioRepositorio.save(clientedb.getUsuario());
                
                // Si el usuario no tiene contraseña (nuevo invitado), enviamos correo de confirmación
                if (clientedb.getUsuario().getPassword() == null) {
                    String token = java.util.UUID.randomUUID().toString();
                    com.veterinaria.modelos.VerificationToken verificationToken = new com.veterinaria.modelos.VerificationToken(
                            token, 
                            clientedb, 
