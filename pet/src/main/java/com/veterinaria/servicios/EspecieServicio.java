...
public class EspecieServicio {
    ...
    public EspecieResponseDTO guardar(EspecieRequestDTO dto) {
        ...
        if (especieRepositorio.existsByNombreIgnoreCase(nombreNormalized)) {
            throw new com.veterinaria.excepciones.BusinessLogicException("La especie ya se encuentra registrada");
        }
        
        Especie especie = new Especie();
        especie.setNombre(nombreNormalized);
    ...
    public EspecieResponseDTO actualizar(Long id, EspecieRequestDTO dto) {
        ...
        if (!especie.getNombre().equalsIgnoreCase(nombreNormalized) && 
            especieRepositorio.existsByNombreIgnoreCase(nombreNormalized)) {
            throw new com.veterinaria.excepciones.BusinessLogicException("La especie ya se encuentra registrada");
        }
